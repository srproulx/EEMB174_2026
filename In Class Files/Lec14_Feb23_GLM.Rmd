...
```{r}
# optional but recommended: center day for nicer geometry/intercept meaning
LACases <- LACases %>%
  mutate(
    day_c = day - mean(day),
...
m.LACases.nb <- ulam(
  alist(
    ...
    # exp(log_scale) ensures positivity; larger scale = more variance
    log_scale ~ dnorm(0, 1)
  ),
  data = LACases,
...
# summarize by day (same structure as your Poisson PPC)
...
# plot: PPC summaries + observed data in red
